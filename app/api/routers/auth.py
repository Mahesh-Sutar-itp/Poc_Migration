...
from app.schemas.auth import LoginRequest, LoginResponse, UserPayload
...
def login(body: LoginRequest, db: Annotated[Session, Depends(get_db)]):
    ...
    user = result["user"]
    return LoginResponse(token=result["token"], user=UserPayload(username=user.username, role=user.role, fullName=user.full_name))
